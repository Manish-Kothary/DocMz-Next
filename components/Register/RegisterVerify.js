...
class RegisterVerify extends Component {
		...
		validateFields((err, values) => {
			...
			if (!err) {
				this.setState(
					...
					() => {
						if (isDoctor) {
							const { repassword, ...restValues } = values;
							const infoDetail = {
								...infoFormDetails,
								basic: {
							...
							const { repassword, ...infoRest } = infoFormDetails;
							const userValues = {
								...infoRest,
								...values
							};
							registerPatient(userValues)
								.then(({ data }) => {
									if (data.status) {
										this.setState({
											isLoading: false
	...
	render() {
		...
		const { infoFormDetails, specialities } = this.props;
						...
						<div className="col-sm-6">
							<FormItem>
								{getFieldDecorator("first_name", {
									rules: [rules.required("Please Input Your First Name")],
						...
						<div className="col-sm-6">
							<FormItem>
								{getFieldDecorator("last_name", {
									rules: [rules.required("Please Input Your Last Name")],
									...
									<div className="col-sm-6">
										<FormItem>
											{getFieldDecorator("phone", {
												rules: [rules.required("Please Input Your Phone")]
											})(
												<Input
													prefix={
														<Icon
